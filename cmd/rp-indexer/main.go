...
func main() {
	...
	idxrs := []indexers.Indexer{
		indexers.NewContactIndexer(cfg.ElasticURL, cfg.ContactsIndex, cfg.ContactsShards, cfg.ContactsReplicas, 500, cfg.RetryIndexBatch),
	}
	...
	if cfg.Rebuild {
		...
		idxr := idxrs[0]
		if _, err := idxr.Index(db, true, cfg.Cleanup, cfg.RetryIndexBatch); err != nil {
