...
func TestContacts(t *testing.T) {
	...
	ix2 := indexers.NewContactIndexer(elasticURL, aliasName, 2, 1, 4, 1)

	indexName2, err := ix2.Index(db, true, false, 1)
	assert.NoError(t, err)
	...
	ix3 := indexers.NewContactIndexer(elasticURL, aliasName, 2, 1, 4, 1)
	indexName3, err := ix3.Index(db, true, true, 1)
	assert.NoError(t, err)
